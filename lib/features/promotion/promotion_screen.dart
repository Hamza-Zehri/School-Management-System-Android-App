...
class _State extends State<PromotionScreen> {
  ...
  Future<void> _promote() async {
    ...
    if (_toClassId == null || _toSectionId == null) { showSnack(context, 'Select destination class and section', isError: true); return; }
    ...
    final ok = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text('Year: $year'),
          const Divider(),
          _previewRow('Promote to next class', counts['promote']!, AppTheme.accent),
          _previewRow('Repeat same class', counts['repeat']!, AppTheme.warning),
          _previewRow('Mark inactive (left)', counts['inactive']!, AppTheme.danger),
          _previewRow('Transfer', counts['transfer']!, AppTheme.info),
          const Divider(),
          const Text('This will update student class/section.\nAll historical records are preserved.', style: TextStyle(fontSize: 12, color: AppTheme.textSecondary)),
        ]),
        actions: [
    ...
    final entries = _students.map((s) => PromotionEntry(
      student: s, action: _actions[s.id!] ?? 'promote',
      newClassId: _toClassId, newSectionId: _toSectionId,
    )).toList();

    final result = await PromotionService.instance.promoteStudents(entries: entries, promotionYear: year);
    setState(() { _loading = false; _students = []; _actions = {}; });
    ...
    if (mounted) {
      showDialog(context: context, builder: (ctx) => AlertDialog(
        ...
        content: Column(mainAxisSize: MainAxisSize.min, children: [
          ...
          _previewRow('Marked Inactive', result.inactive, AppTheme.danger),
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: ListView(padding: const EdgeInsets.all(16), children: [
        ...
        TextFormField(controller: _yearCtrl, keyboardType: TextInputType.number, decoration: const InputDecoration(labelText: 'Promotion Year', hintText: '2025')),
        ...
        if (_students.isNotEmpty) ...[
            ...
            child: Padding(
              ...
              child: Row(children: [
                CircleAvatar(radius: 16, backgroundColor: AppTheme.primary.withOpacity(0.1),
                  child: Text(s.fullName.substring(0,1), style: const TextStyle(color: AppTheme.primary, fontSize: 12))),
                const SizedBox(width: 10),
                Expanded(child: Text(s.fullName, style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w500))),
                DropdownButton<String>(
                  ...
                  items: const [
                    ...
                    DropdownMenuItem(value: 'transfer', child: Text('Transfer', style: TextStyle(color: AppTheme.info, fontSize: 12))),
                  ],
                  onChanged: (v) => setState(() => _actions[s.id!] = v ?? 'promote'),
                ),
              ]),
            ),
          )),
          ...
          SizedBox(width: double.infinity, height: 52,
            child: ElevatedButton.icon(
              ...
              label: _loading ? const CircularProgressIndicator(color: Colors.white, strokeWidth: 2) : const Text('Run Promotion', style: TextStyle(fontSize: 16)),
            ),
          ),
        ],
