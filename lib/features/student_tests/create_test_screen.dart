...
class _State extends State<CreateTestScreen> {
  ...
  Future<void> _onClassChanged(int? v) async {
    ...
    if (v != null) {
      final s =
          await ExtendedDatabaseHelper.instance.getSectionsByClass(v);
      final sub =
          await ExtendedDatabaseHelper.instance.getSubjectsByClass(v);
      setState(() {
        _sections = s;
        _subjects = sub;
      });
  ...
  Future<void> _loadStudents() async {
    if (_classId == null || _sectionId == null) {
      showSnack(context, 'Select class and section', isError: true);
      return;
    }
    final students = await ExtendedDatabaseHelper.instance
        .getStudentsByClassSection(_classId!, _sectionId!, isActive: true);
    // Init controllers
    _totalCtrl.clear();
    _obtainedCtrl.clear();
    _remarksCtrl.clear();
  ...
  Future<void> _save() async {
    if (!_studentsLoaded || _students.isEmpty) {
      showSnack(context, 'Load students first', isError: true);
      return;
    }
    if (_subjectId == null) {
      showSnack(context, 'Select a subject', isError: true);
      return;
    }
    ...
    for (final s in _students) {
      ...
      if (total == null || obtained == null) {
        showSnack(context, 'Enter valid marks for all students', isError: true);
        return;
      }
      if (obtained > total) {
        showSnack(context, '${s.fullName}: Obtained cannot exceed Total marks',
            isError: true);
        return;
      }
    }
    ...
    final marks = _students
        .map((s) => StudentTestMark(
              testId: 0, // will be set by service
              studentId: s.id!,
              totalMarks:
                  double.tryParse(_totalCtrl[s.id!]?.text ?? '10') ?? 10,
              obtainedMarks:
                  double.tryParse(_obtainedCtrl[s.id!]?.text ?? '0') ?? 0,
              remarks: _remarksCtrl[s.id!]?.text.trim().isEmpty == true
                  ? null
                  : _remarksCtrl[s.id!]?.text.trim(),
    ...
    final result = await StudentTestService.instance
        .saveTestWithMarks(test: test, marks: marks, sendSms: _sendSms);

    setState(() => _saving = false);
    if (mounted) {
      showSnack(context,
          'Test saved! ${_sendSms ? "SMS: ${result['smsSent']} sent, ${result['smsFailed']} failed" : ""}');
      ...
      body: ListView(padding: const EdgeInsets.all(16), children: [
        ...
        TextFormField(
            ...
            decoration: const InputDecoration(
                ...
                hintText: 'e.g. Unit Test 1, Surprise Quiz')),
        ...
        if (_studentsLoaded) ...[
          ...
          Row(children: [
            ...
            const SizedBox(width: 8),
            const Text('Send result SMS to guardians',
